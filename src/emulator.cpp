...
static void Execute(const std::filesystem::path& game_patch,
	                const std::vector<std::string>& guest_args) {
	ExecuteRequest request {game_patch, &guest_args};
	Common::Thread guest_thread(
	    [](void* param) {
		    auto* rt = Common::Singleton<Loader::RuntimeLinker>::Instance();
		    const auto* request = static_cast<const ExecuteRequest*>(param);
		    rt->Execute(request->game_patch, *request->guest_args);
	    },
	    &request);
...
void Run(const RunOptions& options) {
	...
	Execute(options.game_patch, options.guest_args);
}
