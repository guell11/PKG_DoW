...
const SymbolRecord* SymbolDatabase::FindByName(const std::string& name, SymbolType type) const {
	...
	// PS5 Payload SDK ELFs use plain FreeBSD-style names. Registered host functions
	// retain their C++ spelling in dbg_name, including the SDK's sceFoo -> Foo form.
	auto plain_name = name;
	if (Common::StartsWith(plain_name, "sce") && plain_name.size() > 3) {
		plain_name = plain_name.substr(3);
	}
	const auto qualified_suffix = "::" + plain_name;
	for (const auto& symbol: m_symbols) {
		if (!Common::EndsWith(symbol.name, suffix)) {
			continue;
		}
		if (symbol.dbg_name == plain_name || Common::EndsWith(symbol.dbg_name, qualified_suffix)) {
			return &symbol;
		}
	}
