...
#include <vector>
#if defined(_WIN32)
// The emulator is built without exceptions; switch xbyak to its checked-error
// mode (SetError/GetError) instead of `throw`.
		...
		struct RelocationSpan {
			...
			const auto emit_span = [&](const RelocationSpan& span) -> std::optional<size_t> {
				...
				for (const auto* decoded: span.instructions) {
					...
					if (const int error = Xbyak::GetError(); error != 0) {
						module->trampoline_gen.setSize(trampoline_offset);
						Xbyak::ClearError();
						if (HandleTrampolineError(module, error)) {
							return std::nullopt;
						}
						LOGF("Windows guest red-zone trampoline emission failed: xbyak error %d\n",
						     error);
						return std::nullopt;
					}
				}
