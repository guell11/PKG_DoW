...
constexpr char ELFOSABI_SYSV    = 0; // System V ABI used by PS5 Payload SDK ELFs

constexpr Elf64_Half ET_DYNEXEC = 0xfe10; // Executable file
constexpr Elf64_Half ET_DYNAMIC = 0xfe18; // Shared
constexpr Elf64_Half ET_SYSV_DYN = 3;     // Standard PIE executable / shared object
...
constexpr Elf64_Word SHT_SYMTAB = 2;
constexpr Elf64_Word SHT_STRTAB = 3;
constexpr Elf64_Word SHT_DYNSYM = 11;
...
public:
	...
	[[nodiscard]] std::optional<Elf64_Addr> FindStaticSymbolValue(std::string_view name) const;
	// Names of all dynamic (undefined, imported) symbols from .dynsym in
	// entry order. The payload rtld resolves the main executable's imports
	// by NID-encoding these names and matching them against each loaded
	// module's symtab.
	[[nodiscard]] std::vector<std::pair<std::string, uint32_t>> GetDynamicImportNames() const;
