...
std::optional<Elf64_Addr> Elf64::FindStaticSymbolValue(std::string_view name) const {
	if (name.empty() || m_f == nullptr || m_f->IsInvalid() || m_self != nullptr || m_ehdr == nullptr ||
	    m_shdr == nullptr) {
		return std::nullopt;
	}

	const auto file_size = m_f->Size();
	for (Elf64_Half i = 0; i < m_ehdr->e_shnum; i++) {
		const auto& symbols = m_shdr[i];
		if (symbols.sh_type != SHT_SYMTAB || symbols.sh_entsize != sizeof(Elf64_Sym) ||
		    symbols.sh_size % sizeof(Elf64_Sym) != 0 || symbols.sh_link >= m_ehdr->e_shnum ||
		    symbols.sh_offset > file_size || symbols.sh_size > file_size - symbols.sh_offset) {
			continue;
		}

		const auto& strings = m_shdr[symbols.sh_link];
		if (strings.sh_type != SHT_STRTAB || strings.sh_size == 0 ||
		    strings.sh_size > std::numeric_limits<uint32_t>::max() || strings.sh_offset > file_size ||
		    strings.sh_size > file_size - strings.sh_offset) {
			continue;
		}

		std::vector<char> string_table(static_cast<size_t>(strings.sh_size));
		m_f->Seek(strings.sh_offset);
		m_f->Read(string_table.data(), static_cast<uint32_t>(string_table.size()));

		for (uint64_t offset = 0; offset < symbols.sh_size; offset += sizeof(Elf64_Sym)) {
			Elf64_Sym symbol {};
			m_f->Seek(symbols.sh_offset + offset);
			m_f->Read(&symbol, sizeof(symbol));
			if (symbol.st_name >= string_table.size()) {
				continue;
			}

			const auto* symbol_name = string_table.data() + symbol.st_name;
			const auto remaining    = string_table.size() - symbol.st_name;
			const auto* terminator = static_cast<const char*>(std::memchr(symbol_name, '\0', remaining));
			if (terminator != nullptr && std::string_view(symbol_name, terminator - symbol_name) == name) {
...
std::vector<std::pair<std::string, uint32_t>> Elf64::GetDynamicImportNames() const {
	std::vector<std::pair<std::string, uint32_t>> imports;
	if (m_f == nullptr || m_f->IsInvalid() || m_ehdr == nullptr || m_shdr == nullptr) {
		return imports;
	}
	...
	for (Elf64_Half i = 0; i < m_ehdr->e_shnum; i++) {
		...
		const auto& strings = m_shdr[symbols.sh_link];
		if (strings.sh_type != SHT_STRTAB || strings.sh_size == 0 ||
		    strings.sh_offset > file_size || strings.sh_size > file_size - strings.sh_offset) {
			continue;
		}
		...
		auto index = static_cast<uint32_t>(symbols.sh_offset / sizeof(Elf64_Sym));
		for (uint64_t offset = 0; offset < symbols.sh_size; offset += sizeof(Elf64_Sym), index++) {
			Elf64_Sym symbol {};
			m_f->Seek(symbols.sh_offset + offset);
			m_f->Read(&symbol, sizeof(symbol));
			if (symbol.st_shndx != SHN_UNDEF || symbol.st_name == 0 ||
			    symbol.st_name >= string_table.size()) {
				continue;
			}
			...
			imports.emplace_back(std::string_view(symbol_name, terminator - symbol_name), index);
		}
		break;
	}
...
bool Elf64::IsValid() const {
	...
	if (m_ehdr->e_ident[EI_OSABI] != ELFOSABI_FREEBSD &&
		...
		LOGF("ehdr->e_ident[EI_OSABI] (0x%x) is neither FreeBSD nor System V\n",
	...
	if (m_ehdr->e_type != ET_DYNEXEC && m_ehdr->e_type != ET_DYNAMIC &&
	    m_ehdr->e_type != ET_SYSV_DYN) {
		LOGF("ehdr->e_type (%04x) is unsupported\n", m_ehdr->e_type);
		return false;
	}
