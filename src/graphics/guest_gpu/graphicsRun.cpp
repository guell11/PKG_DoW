...
template <typename T>
static void RecordLabelProduced(void* dst_gpu_addr, T value);
...
void CommandProcessor::WaitRegMem(uint32_t func, const T* addr, T ref, T mask, uint32_t poll,
	...
	(void)poll;
	WaitRegistry::Waiter waiter {};
	waiter.address   = reinterpret_cast<uint64_t>(addr);
	waiter.reference = static_cast<uint64_t>(ref);
	waiter.mask      = static_cast<uint64_t>(mask);
	waiter.function  = func;
	waiter.is64      = sizeof(T) == sizeof(uint64_t);

	// A producer may have satisfied this exact waiter since its last re-poll
	// and the label may since have been reset by the guest; the latch keeps the
	// wakeup from being lost.
	WaitRegistry::Register(waiter);
	if (!WaitRegistry::CollectSatisfied().empty()) {
		// Satisfied through the latch: CollectSatisfied already removed this
		// waiter, so fall through without suspending.
		return;
	}
...
void CommandProcessor::WriteReferenceClock(uint64_t dst_address, uint32_t num_bytes) {
	...
	WaitRegistry::RecordProduced(dst_address, value, num_bytes == sizeof(uint64_t));
...
void GuestGpu::WakeBlockedSubmissions() {
	if (g_gpu_state == nullptr) {
		return;
	}
	// SignalAll rather than Signal: the GPU thread may be parked in the
	// blocked-queue re-poll wait while other waiters exist; waking all is
	// cheap because the loop re-checks its predicates under the mutex.
	g_gpu_state->m_work_available.SignalAll();
}
	...
	for (;;) {
		...
		bool                         should_stop    = false;
		{
			...
			if (gpu->m_stopping && gpu->m_commands.empty() && gpu->m_submission_count == 0) {
				...
				if (selected_queue < 0) {
					gpu->m_processing = false;
					// Blocked re-poll. Producers that write a waited label call
					// GpuWakeNudge and unblock this wait immediately; the timeout
					// only bounds the worst case when no producer ever signals.
					gpu->m_work_available.WaitFor(&gpu->m_queue_mutex, 100);
					static uint64_t last_progress_tick = WaitRegistry::Now();
					if (WaitRegistry::Now() - last_progress_tick > kWakeNudgeMinAgeMicros) {
						const auto now = WaitRegistry::Now();
						for (auto& waiter: WaitRegistry::CollectDeadlockBroken(
						         now, kWakeNudgeMinAgeMicros)) {
							(void)waiter;
						}
...
bool GuestGpu::Process(Submission& submission) {
	...
	switch (submission.type) {
		...
		case SubmissionType::FlipPreparation:
			m_renderer.GetGpuResources().RunGarbageCollector();
			// Frame boundary: producer label writes from the previous frame must
			// not satisfy the current frame's waits through the latch path.
			WaitRegistry::AdvanceFrame();
...
template <typename T>
static void RecordLabelProduced(void* dst_gpu_addr, T value) {
	if (WaitRegistry::RecordProduced(reinterpret_cast<uint64_t>(dst_gpu_addr),
...
void CommandProcessor::Flip(void* dst_gpu_addr, uint32_t value) {
	...
	RecordLabelProduced(dst_gpu_addr, value);
	auto& command = CurrentBuffer();
	auto request = Sync::PrepareVideoOutFlip(command, m_flip.handle, m_flip.index, m_flip.flip_mode,
	                                         m_flip.flip_arg);
