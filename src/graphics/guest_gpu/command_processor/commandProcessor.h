...
private:
	...
	// Async-compute queues record through the dedicated compute scheduler; the
	// graphics queue and flip preparation keep the graphics scheduler. Both
	// share one master timeline, so ticks stay globally ordered.
	CommandScheduler&   GetScheduler() const {
		return IsAsyncComputeQueue() ? m_renderer.GetComputeCommandScheduler()
		                              : m_renderer.GetCommandScheduler();
	}
