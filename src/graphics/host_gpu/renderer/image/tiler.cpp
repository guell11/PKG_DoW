...
TileManager::Scratch TileManager::AllocateScratch(uint64_t size) {
	EXIT_IF(size == 0);
	// Deferred destroys only run once the scheduler's GPU tick passes the
	// queued tick; guests that upload thousands of textures in a burst
	// (Doom's startup) can exhaust device memory long before the timeline
	// semaphore catches up. When too many scratch buffers are alive, block
	// until the pending destroy queue drains instead of allocating more.
	constexpr uint64_t kMaxOutstandingScratch = 48;
	if (m_scratch_outstanding >= kMaxOutstandingScratch) {
		LOGF("TileManager: draining %llu deferred scratch buffers (VRAM pressure)\n",
		     static_cast<unsigned long long>(m_scratch_outstanding));
		m_scheduler.Flush();
	}
...
void TileManager::Record(bool tile, vk::Buffer source, uint64_t source_offset,
	...
	auto&                   scheduler = CommandScheduler::HasRecording()
	                                        ? CommandScheduler::Recording()
	                                        : static_cast<CommandScheduler&>(m_scheduler);
...
void TileManager::ConvertD16(Result source, Result target, D16Direction direction, bool d32,
	...
	auto& scheduler = CommandScheduler::HasRecording() ? CommandScheduler::Recording()
	                                                   : static_cast<CommandScheduler&>(m_scheduler);
	scheduler.EndRendering();
	auto                    command = scheduler.Current().Handle();
