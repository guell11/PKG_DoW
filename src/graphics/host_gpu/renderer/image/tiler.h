...
private:
	...
	void                       SwapBgra16(Result input, Result output, uint32_t pixels);
		...
		// Number of scratch buffers handed out whose deferred destroy has
		// not been queued yet; used to drain the scheduler under VRAM
		// pressure (see AllocateScratch).
		uint64_t                                m_scratch_outstanding = 0;
