...
// Scheduler for the submission currently being recorded: image uploads and
// layout transitions taken during an async-compute submission must record on
// that scheduler's command buffer, not the graphics one the image was created
// with.
CommandScheduler& ActiveScheduler(CommandScheduler* fallback) {
	...
	if (partial || has_subresource_states) {
		...
		for (uint32_t level = base_level; level < base_level + level_count; level++) {
			for (uint32_t layer = base_layer; layer < base_layer + layer_count; layer++) {
				...
				const bool has_write = static_cast<bool>(
				    (subresource_state.access_mask | destination_access) & write_access);
				const bool attachment_write =
				    static_cast<bool>(destination_access &
				                      (vk::AccessFlagBits2::eColorAttachmentWrite |
				                       vk::AccessFlagBits2::eDepthStencilAttachmentWrite)) &&
				    !static_cast<bool>(destination_access &
				...
				if (subresource_state.layout == destination_layout &&
				    ...
				    (!has_write || attachment_write)) {
					// Identical read scopes and pinned attachment scopes need no additional
					// dependency. Merge stages instead of flushing the pipeline.
					subresource_state.pl_stage |= destination_stage;
					subresource_state.access_mask |= destination_access;
					continue;
				}
		...
		const bool has_write =
		    static_cast<bool>((state.access_mask | destination_access) & write_access);
		const bool attachment_write =
		    static_cast<bool>(destination_access &
				                      (vk::AccessFlagBits2::eColorAttachmentWrite |
				                       vk::AccessFlagBits2::eDepthStencilAttachmentWrite)) &&
...
void Image::Transit(vk::ImageLayout destination_layout, vk::AccessFlags2 destination_access,
	...
	if (m_scheduler != nullptr) {
		ActiveScheduler(m_scheduler).EndRendering();
	}
...
void Image::Upload(std::span<const vk::BufferImageCopy> copies, vk::Buffer buffer, uint64_t offset,
	...
	auto command                        = ActiveScheduler(m_scheduler).Current().Handle();
