...
CommandScheduler& Buffer::Scheduler() const noexcept {
	// Buffers are shared between the graphics and async-compute schedulers:
	// route command-buffer side effects to the scheduler currently recording
	// so copies land in the submission that consumes them.
	return CommandScheduler::HasRecording() ? CommandScheduler::Recording() : *m_scheduler;
}
