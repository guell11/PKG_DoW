...
protected:
	...
	// Defined out-of-line in streamBuffer.cpp: buffers are shared between the
	// graphics and async-compute schedulers, so command-buffer side effects
	// route to the scheduler currently recording and copies land in the
	// submission that consumes them.
	[[nodiscard]] CommandScheduler& Scheduler() const noexcept;
