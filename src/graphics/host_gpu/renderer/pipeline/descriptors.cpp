...
static BufferView NativeUpload(RenderContext& context, std::span<const uint32_t> data) {
	EXIT_IF(data.empty());
	// Descriptor payloads upload onto the command buffer of the submission
	// being recorded: compute dispatches come from the async-compute
	// scheduler, so the utility copy must not land on the graphics queue.
	auto& scheduler = CommandScheduler::HasRecording() ? CommandScheduler::Recording()
	                                                   : context.GetCommandScheduler();
...
void RenderExecutor::CommitBindings(CommandBuffer&                     buffer,
	...
	if (!m_descriptor_writes.empty()) {
		...
		if (pipeline.uses_push_descriptors) {
			...
			const auto cached = m_context.GetDescriptorHeap().CommitCached(
			    pipeline.descriptor_set_layout, m_descriptor_writes);
			const auto set = cached.set;
