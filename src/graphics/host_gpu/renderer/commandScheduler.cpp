...
namespace {
// Scheduler whose submission is being recorded right now. Recording is
// serialized on the guest GPU thread; other threads see no recording and
// fall back to the graphics scheduler.
thread_local Libs::Graphics::CommandScheduler* g_recording_scheduler = nullptr;
} // namespace

CommandScheduler& CommandScheduler::Recording() {
	auto* scheduler = g_recording_scheduler;
	EXIT_IF(scheduler == nullptr);
	return *scheduler;
}

bool CommandScheduler::HasRecording() noexcept {
	return g_recording_scheduler != nullptr;
}

CommandScheduler::CommandScheduler(RenderContext& context, GraphicContext& graphics)
    : m_owned_master(std::make_unique<MasterSemaphore>(graphics)), m_master(*m_owned_master),
      m_context(context), m_graphics(graphics),
      m_command_pool(graphics, m_master, graphics.queue_family), m_submit_queue(graphics.queue),
      m_command(*this),
      m_priority_thread([this](std::stop_token stop) { PriorityOperationsThread(stop); }) {}

CommandScheduler::CommandScheduler(RenderContext& context, GraphicContext& graphics,
                                   MasterSemaphore& master, bool compute_family)
    : m_master(master), m_context(context), m_graphics(graphics),
      m_command_pool(graphics, m_master,
                     compute_family ? graphics.compute_queue_family : graphics.queue_family),
      m_submit_queue(compute_family ? graphics.compute_queue : graphics.queue),
      m_command(*this),
      m_priority_thread([this](std::stop_token stop) { PriorityOperationsThread(stop); }) {}
...
void CommandScheduler::Shutdown() {
	{
		...
		if (g_recording_scheduler == this) {
			g_recording_scheduler = nullptr;
		}
	}
	{
...
void CommandScheduler::Flush(SubmitInfo& submit) {
	Submit(submit);
	BeginNext();
	// A flush is a GPU operation boundary: resources deferred for
	// destruction after older ticks may already be free. Popping here
	// keeps the pending queue bounded for CPU-rendered guests (payload
	// homebrew) that never call Finish() between frames.
	PopPendingOperations();
}
...
uint64_t CommandScheduler::Submit(SubmitInfo submit) {
	...
	EXIT_IF(m_submit_queue == nullptr);
	...
	uint64_t   tick;
	{
		...
		// Cross-queue ordering: wait for the previous globally-ordered tick
		// before this submission runs. Both schedulers share one timeline, so
		// a compute submission records after the graphics work that produced
		// its inputs and the graphics queue observes compute results it
		// consumes.
		const uint64_t previous_tick = tick - 1;
		if (previous_tick > 0) {
			submit.AddWait(m_master.Handle(), previous_tick,
		...
		result = m_submit_queue.submit(1, &submit_info, nullptr);
	}
