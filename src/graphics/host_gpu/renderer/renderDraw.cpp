...
void RenderExecutor::ExecutePreparedDraw(uint64_t submit_id, CommandBuffer& buffer,
	...
	if (log_pipeline_phase) {
		LogDrawPhase(draw.name, "BeginPipelineCompile");
	}
	auto pipeline_ticket = m_context.GetPipelineCache().BeginGraphicsPipeline(
	...
	if (log_pipeline_phase) {
		LogDrawPhase(draw.name, "WaitPipelineCompile");
	}
	auto& pipeline = pipeline_ticket.Wait();
	...
	buffer.GetScheduler().BeginRendering(state.rendering);
	...
	if (shader_write_stages) {
		buffer.GetScheduler().EndRendering();
...
void RenderExecutor::DrawIndex(uint64_t submit_id, CommandBuffer& buffer,
	...
	buffer.GetScheduler().PopPendingOperations();
	auto& ucfg   = buffer.GetUserConfig();
	auto& sh_ctx = buffer.GetShaders();
