...
public:
	...
	// The scheduler this buffer was committed by. Compute submissions come
	// from the async-compute scheduler; every scheduler-side effect taken on
	// behalf of this buffer (rendering state, pending operations, EOP
	// signals) must route through it instead of the graphics one.
	[[nodiscard]] CommandScheduler& GetScheduler() const noexcept { return *m_scheduler; }
