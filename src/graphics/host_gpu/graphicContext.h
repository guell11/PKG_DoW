...
enum class HostGpuArchitecture {
	Generic,
	NvidiaTuring,
	NvidiaAmpere,
	NvidiaAda,
...
struct HostGpuTuningProfile {
	HostGpuArchitecture architecture            = HostGpuArchitecture::Generic;
	uint32_t            descriptor_cache_entries = 4096;
	uint32_t            pipeline_worker_count    = 1;
	float               vram_target_ratio        = 0.88f;
	float               vram_pressure_ratio      = 0.93f;
	float               vram_critical_ratio      = 0.97f;
};
...
struct GraphicContext {
	...
	// Dedicated async-compute queue (own family when the device exposes one,
	// otherwise the same family as the graphics queue). Sharing the master
	// timeline semaphore keeps cross-queue submission order host-visible.
	...
	[[nodiscard]] HostGpuTuningProfile GetHostGpuTuningProfile() const;
	[[nodiscard]] DeviceMemoryBudget   GetAdaptiveDeviceMemoryBudget() const;
	[[nodiscard]] const char*          GetHostGpuArchitectureName() const;
