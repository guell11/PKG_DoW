	...
	if (op == IR::ValueOpcode::ImageSampleRaw || op == IR::ValueOpcode::ImageGatherRaw) {
		...
		if (op == IR::ValueOpcode::ImageGatherRaw) {
			if (view == ImageViewKind::Dim1D) {
				...
				ctx.Define(inst,
				           ResultVector(ctx, DecodeImageSrgb(ctx, mem, UnpackImageGather(ctx, mem, sample),
				                                             true),
				                        integer, false, mem, true));
				return true;
			}
		...
		if (image.indirect_root != mem.resource) {
			...
			ctx.Define(inst,
			           ResultVector(ctx,
			                        dref ? sample
			                             : DecodeImageSrgb(ctx, mem, UnpackImageTexel(ctx, mem, sample),
			                                               false),
			                        integer, dref, mem));
			return true;
		}
