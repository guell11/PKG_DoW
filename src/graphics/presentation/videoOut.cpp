...
bool FlipQueue::Flip(uint32_t micros) {
	...
	if (m_requests.front().state != RequestState::Ready) {
		LOGF("[fliptrace] front-not-ready request=%" PRIu64 " state=%d t=%" PRIu64 "\n",
		     m_requests.front().id, static_cast<int>(m_requests.front().state),
		     static_cast<uint64_t>(Common::Timer::QueryPerformanceCounter()));
		m_mutex.Unlock();
		return false;
	}
	m_processing = true;
	...
	const auto present_t0 = Common::Timer::QueryPerformanceCounter();
	m_presenter.Present(*r.frame);
	LOGF("[fliptrace] presented request=%" PRIu64 " t=%" PRIu64 " dt_us=%" PRIu64 "\n", r.id,
	     static_cast<uint64_t>(present_t0),
	     static_cast<uint64_t>((Common::Timer::QueryPerformanceCounter() - present_t0) *
	                           1000000u / Common::Timer::QueryPerformanceFrequency()));
	...
	LOGF("[fliptrace] triggered request=%" PRIu64 " t=%" PRIu64 "\n", r.id,
	     static_cast<uint64_t>(Common::Timer::QueryPerformanceCounter()));
...
KYTY_SYSV_ABI int VideoOutSubmitFlip(int handle, int index, int flip_mode, int64_t flip_arg) {
	...
	LOGF("[fliptrace] submit request=%" PRIu64 " t=%" PRIu64 " tid=%d\n", request_id,
	     static_cast<uint64_t>(Common::Timer::QueryPerformanceCounter()),
...
void VideoOutDriver::PrepareFlip(uint64_t request_id, Graphics::CommandBuffer& buffer) {
	LOGF("[fliptrace] prepare request=%" PRIu64 " t=%" PRIu64 "\n", request_id,
	     static_cast<uint64_t>(Common::Timer::QueryPerformanceCounter()));
	m_impl->GetFlipQueue().Prepare(request_id, buffer);
}

void VideoOutDriver::CompleteFlip(uint64_t request_id) {
	LOGF("[fliptrace] ready request=%" PRIu64 " t=%" PRIu64 "\n", request_id,
	     static_cast<uint64_t>(Common::Timer::QueryPerformanceCounter()));
