...
static void FocusWindow(SDL_Window* window) {
	if (window == nullptr) {
		return;
	}
	...
	if (SDL_GetWindowWMInfo(window, &wm_info) == SDL_TRUE &&
	    wm_info.subsystem == SDL_SYSWM_WINDOWS && wm_info.info.win.window != nullptr) {
		const auto hwnd = wm_info.info.win.window;
...
static void GameEventKeyboard(WindowLoopState& game, const EventKeyboard& key) {
	...
	const bool alt_enter =
	    (key.key_code == SDLK_RETURN || key.key_code == SDLK_KP_ENTER) &&
	    (key.mod & KMOD_ALT) != 0;
	const bool fullscreen_shortcut = key.key_code == SDLK_F11 || alt_enter;

	if (key.down && !key.repeat) {
		switch (key.key_code) {
			case SDLK_SPACE: SetPause(game, !game.paused.load(std::memory_order_acquire)); break;
			case SDLK_F1: RenderDocRequestCapture(); break;
			default: break;
		}
	}

	if (key.down && !key.repeat && fullscreen_shortcut) {
		ToggleDesktopFullscreen();
		fullscreen_key = key.key_code;
		return;
	}

	if (key.key_code == fullscreen_key || key.key_code == SDLK_F11) {
		if (key.up && key.key_code == fullscreen_key) {
