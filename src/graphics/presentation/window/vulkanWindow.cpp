...
static vk::Device VulkanCreateDevice(vk::PhysicalDevice physical_device, const VulkanExtensions& r,
	...
	// Dedicated async-compute family: a separate queue lets pure-compute
	// submissions run without blocking the graphics queue. Falls back to the
	// graphics family (same queue) when the device has no dedicated compute
	// family — the master timeline semaphore keeps cross-queue ordering
	// host-visible either way.
	uint32_t compute_queue_family = queue_family;
	{
	...
	std::vector<vk::DeviceQueueCreateInfo> queue_create_infos {};
	{
		...
		graphics_queue_info.sType            = vk::StructureType::eDeviceQueueCreateInfo;
		graphics_queue_info.queueFamilyIndex = queue_family;
	...
	if (compute_queue_family != queue_family) {
		...
		compute_queue_info.sType            = vk::StructureType::eDeviceQueueCreateInfo;
		compute_queue_info.queueFamilyIndex = compute_queue_family;
		...
		LOGF("Vulkan async compute queue: family %u (graphics family %u)\n", compute_queue_family,
		     queue_family);
	}
	...
	graphics.compute_queue_family = compute_queue_family;
	device.getQueue(compute_queue_family, 0, &graphics.compute_queue);
	EXIT_IF(graphics.compute_queue == nullptr);
	return device;
}
...
void WindowContext::CreateVulkan() {
	...
	graphic_ctx.device.getQueue(graphic_ctx.compute_queue_family, 0, &graphic_ctx.compute_queue);
	EXIT_IF(graphic_ctx.compute_queue == nullptr);
