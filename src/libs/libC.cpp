...
int KYTY_SYSV_ABI fflush(FILE* stream) {
	...
	// Guest libc may flush any stream it opened (stderr, WAD handles stored
	// in FILE wrappers); flushing through the host CRT is safe for all of
	// them, so only unknown FILE pointers fall back to a no-op.
...
const char* KYTY_SYSV_ABI strchr(const char* s, int c) {
	// The PS5 payload rtld probes module names that can still be null
	// while it assembles its dependency graph; the retail libc answers
	// null with null instead of faulting.
	if (s == nullptr) {
		return nullptr;
	}
	return ::strchr(s, c);
