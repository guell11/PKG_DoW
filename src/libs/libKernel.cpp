...
int KYTY_SYSV_ABI KernelDlsym(KernelModule handle, const char* symbol, void** addr) {
	...
	if (program == nullptr || program->export_symbols == nullptr) {
		// Fake handles returned for HLE'd system modules: resolve from the
		// global database so payload CRTs can dlsym through them.
		const auto* record = kernel_find_export_symbol(rt->Symbols(), symbol);
		if (record != nullptr) {
			*addr = reinterpret_cast<void*>(record->vaddr);
			LOGF("\t resolved (system-module HLE) = 0x%016" PRIx64 " (%s)\n", record->vaddr,
			     symbol);
			return OK;
		}
		*addr = nullptr;
...
static bool IsHleSystemModulePath(std::string_view path) {
	return (Common::StartsWith(path, "/system/") || Common::StartsWith(path, "/system_ex/") ||
	        Common::StartsWith(path, "/user/homebrew/")) &&
	       path.find("/lib/") != std::string_view::npos && Common::EndsWith(path, ".sprx");
}
...
static KYTY_SYSV_ABI KernelModule KernelLoadStartModule(const char* module_file_name, size_t args,
	...
	// System modules are fully HLE'd; payload SDK CRTs only need a usable handle
	// from the load call. The loader builds a fake kernel dynlib record whose
	// node ids are 0x12345001 + the module's index in that record's name list,
	// so the handle must be derived from the module name (not a load-order
	// counter) for the CRT's kernel_dynlib_obj walk to find the node.
	static constexpr const char* kPayloadDynlibNames[] = {
	...
	static constexpr uint32_t kUnknownDynlibIndex = 0xffffffffu;
	uint32_t payload_dynlib_index    = kUnknownDynlibIndex;
	if (module_file_name != nullptr) {
		const std::string_view module {module_file_name};
		const auto slash = module.find_last_of('/');
		const std::string_view basename =
		    slash == std::string_view::npos ? module : module.substr(slash + 1);
		for (size_t i = 0; i < std::size(kPayloadDynlibNames); i++) {
			std::string_view known {kPayloadDynlibNames[i]};
			const auto known_slash = known.find_last_of('/');
			const std::string_view known_basename =
			    known_slash == std::string_view::npos ? known : known.substr(known_slash + 1);
			if (basename == known_basename) {
				payload_dynlib_index = static_cast<uint32_t>(i);
				break;
			}
		}
	}
	if (!Common::File::IsFileExisting(module_path)) {
		...
		if (module_file_name != nullptr && IsHleSystemModulePath(module_file_name)) {
			// HLE-only system module: stable name-derived handle that matches the
			// loader's fake dynlib record (node id = 0x12345001 + index).
			const auto kFakeSystemModuleHandle = payload_dynlib_index != kUnknownDynlibIndex
			                                       ? static_cast<KernelModule>(0x12345001u +
			                                                                   payload_dynlib_index)
			                                       : static_cast<KernelModule>(0x1234f000u);
			if (res != nullptr) {
				*res = OK;
			}
			LOGF("\tsystem module provided by HLE: %s -> handle %" PRIu32 "\n", module_file_name,
			     static_cast<uint32_t>(kFakeSystemModuleHandle));
			return kFakeSystemModuleHandle;
		}
		if (res != nullptr) {
	...
	if (program != nullptr) {
		...
		// The adjacent stub .sprx programs exist on disk, so they load through
		// the normal linker path here. Payload CRTs look the module up in the
		// fake kernel dynlib record by this handle, so override the linker id
		// with the stable name-derived id for the known system modules.
		if (payload_dynlib_index != kUnknownDynlibIndex) {
			const auto kFakeSystemModuleHandle =
			    static_cast<KernelModule>(0x12345001u + payload_dynlib_index);
			LOGF("\tsystem module handle overridden for payload dynlib walk: %s -> %" PRIu32 "\n",
			     module_file_name, static_cast<uint32_t>(kFakeSystemModuleHandle));
			return kFakeSystemModuleHandle;
		}
	...
	if (payload_dynlib_index != kUnknownDynlibIndex) {
		handle = 0x12345001u + payload_dynlib_index;
		LOGF("\tsystem module handle overridden for payload dynlib walk: %s -> %" PRIu32 "\n",
		     module_file_name, static_cast<uint32_t>(handle));
	}
