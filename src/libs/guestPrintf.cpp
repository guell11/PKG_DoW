...
static int kyty_printf_internal(bool sn, char* sn_s, size_t sn_n, const char* format,
	...
	while (*format != 0) {
		...
		switch (*format) {
			...
			case 's': {
				...
				// The retail libc prints "(null)" for null string arguments;
				// Doom's error paths rely on that instead of crashing.
				if (p == nullptr) {
					p = "(null)";
				}
				unsigned int l =
	...
	if (sn) {
		if (sn_n == 0) {
			// C standard: snprintf/vsnprintf with size 0 writes nothing.
			return static_cast<int>(idx);
		}
		const auto copy =
		    (static_cast<size_t>(idx) < sn_n) ? static_cast<size_t>(idx) : sn_n - 1U;
...
int GuestVsnprintfVa(char* s, size_t n, const char* format, VaList* va) {
	return kyty_printf_internal(true, s, n, format, va);
}
