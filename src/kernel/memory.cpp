...
public:
	...
	// The console exposes ~13.5 GB of direct memory. Windows commits guest backing
	// pages on demand; KYTY_DIRECT_MEMORY_MB can lower the capacity when required.
	static uint64_t TotalSize() {
		static const uint64_t configured = [] {
			uint64_t total = static_cast<uint64_t>(13824) * 1024 * 1024;
			if (const char* env = std::getenv("KYTY_DIRECT_MEMORY_MB");
			    env != nullptr && env[0] != '\0') {
				char* end = nullptr;
				errno    = 0;
				const auto requested = std::strtoull(env, &end, 10);
				const auto min_mb = g_flexible_memory_size / (1024 * 1024) + 1;
				if (errno == 0 && end != env && *end == '\0' && requested >= min_mb &&
				    requested < 13824) {
					total = static_cast<uint64_t>(requested) * 1024 * 1024;
					LOGF("Direct memory override: %" PRIu64 " MB (KYTY_DIRECT_MEMORY_MB)\n",
					     static_cast<uint64_t>(requested));
				} else {
					LOGF_COLOR(Log::Color::Yellow,
					           "Ignoring KYTY_DIRECT_MEMORY_MB='%s' (expected %" PRIu64
					           "..13823 MB, above flexible memory)\n",
					           env, min_mb);
				}
			}
			return total;
...
int KYTY_SYSV_ABI KernelSetVirtualRangeName(const void* addr, uint64_t len, const char* name) {
	...
	LOGF_EVERY(256, "\t addr = 0x%016" PRIx64 "\n"
	     "\t len  = 0x%016" PRIx64 "\n"
	     "\t name = %s\n",
