...
#if KYTY_PLATFORM == KYTY_PLATFORM_WINDOWS
// Samples every live guest thread's RIP once per interval; consecutive dumps
// with identical RIPs pinpoint exactly where the game wedged.
static void PthreadWatchdog(void* /*arg*/) {
	PRINT_NAME_ENABLE(false);

	auto* pthread_pool = g_pthread_context->GetPthreadPool();

	Common::Thread::Sleep(15000);
	while (true) {
		LOGF("[watchdog] === begin dump ===\n");
		pthread_pool->DumpThreads();
		LOGF("[watchdog] === end dump ===\n");
		fflush(stdout);
		Common::Thread::Sleep(5000);
	}
}
#endif
...
#if KYTY_PLATFORM == KYTY_PLATFORM_WINDOWS
	Common::Thread watchdog(PthreadWatchdog, nullptr);
	watchdog.Detach();
#endif
}
...
static void PthreadAttrDbgPrint(const PthreadAttr* src) {
	...
	LOGF_EVERY(256, "\tcpu_mask       = 0x%" PRIx64 "\n"
	     "\tdetach_state   = %d\n"
	     "\tguard_size     = %" PRIu64 "\n"
	     "\tinherit_sched  = %d\n"
	     "\tsched_priority = %d\n"
...
#if KYTY_PLATFORM == KYTY_PLATFORM_WINDOWS
// Freeze watchdog: samples RIP of every live guest thread. A wedged loop
// (deadlock, spin, unresolved wait) shows a frozen RIP; per-thread symbol
// resolution is unnecessary — the address range identifies the module.
void PthreadPool::DumpThreads() {
	Common::LockGuard lock(m_mutex);
	...
	for (auto* p: m_threads) {
		...
		const HANDLE handle = static_cast<HANDLE>(pthread_gethandle(p->p));
		if (handle == nullptr) {
			continue;
		}
		...
		const bool have_ctx = GetThreadContext(handle, &ctx) != 0;
		ResumeThread(handle);
		if (have_ctx) {
			LOGF("[watchdog] tid=%d name='%s' rip=%016llx rsp=%016llx\n",
			     p->guest.thread_id, p->name.c_str(),
...
static void* RunThread(void* arg) {
	...
	LOGF_EVERY(256, "\tPthread run begin: %s, id = %d, os_thread_id = %" PRIu64 ", entry = 0x%016" PRIx64
...
int KYTY_SYSV_ABI PthreadCreate(Pthread* thread, const PthreadAttr* attr,
	...
	*thread              = created_thread;
	...
	LOGF_EVERY(256, "\tthread create: %s, id = %d, %d\n", created_thread->name.c_str(),
...
int KYTY_SYSV_ABI PthreadJoin(Pthread thread, void** value) {
	...
	if (PRINT_NAME_ENABLED) {
		LOGF_EVERY(256, "\tthread join: %s, %d\n", thread->name.c_str(), result);
	}
...
unsigned int KYTY_SYSV_ABI KernelSleep(unsigned int seconds) {
	...
	double ts = t.GetTimeS();
	LOGF_EVERY(4096, "\tactual: %g seconds\n", ts);
	return OK;
}
...
int KYTY_SYSV_ABI KernelNanosleep(const KernelTimespec* rqtp, KernelTimespec* rmtp) {
	...
	// Game-loop pacing probe: one line per second of steady-state activity.
	// Nanosleep rate tracks the guest's main-loop frequency directly.
	{
		static std::atomic_uint64_t sleep_total {0};
		static std::atomic_uint64_t sleep_second {0};
		const auto    ticks  = Common::Timer::QueryPerformanceCounter();
		const auto    freq   = Common::Timer::QueryPerformanceFrequency();
		const uint64_t second = freq > 0 ? static_cast<uint64_t>(ticks / freq) : 0;
		const uint64_t total  = sleep_total.fetch_add(1, std::memory_order_relaxed) + 1;
		if (sleep_second.exchange(second, std::memory_order_relaxed) != second) {
			LOGF("[loop] nanosleeps=%llu in second %llu\n",
			     static_cast<unsigned long long>(total), static_cast<unsigned long long>(second));
			sleep_total.store(0, std::memory_order_relaxed);
		}
	}
	...
	LOGF_EVERY(4096, "\tnanosleep: %" PRIu64 "\n", nanos);

	Common::Timer t;
	t.Start();
	SleepNanoWithSignalPoll(nanos);
	double ts = t.GetTimeS();
	LOGF_EVERY(4096, "\tactual: %g nanoseconds\n", ts * 1000000000.0);
