...
static std::string ResolveGuestRelativePath(const std::string& path) {
	auto fixed_path = Common::FixFilenameSlash(path);
	if (!fixed_path.empty() && fixed_path.front() != '/' &&
	    !(fixed_path.size() >= 2 && fixed_path[1] == ':')) {
		return "/app0/" + fixed_path;
	}
	return fixed_path;
}
...
std::filesystem::path MountPoints::GetRealFilename(const std::string& mounted_file_name) {
	...
	const auto guest_path   = ResolveGuestRelativePath(mounted_file_name);
	auto       mounted_path =
	    Common::FixDirectorySlash(Common::DirectoryWithoutFilename(guest_path));
...
std::filesystem::path MountPoints::GetRealDirectory(const std::string& mounted_directory) {
	...
	const auto guest_path   = Common::FixDirectorySlash(ResolveGuestRelativePath(mounted_directory));
	auto       mounted_path = guest_path;
...
int64_t KYTY_SYSV_ABI KernelRead(int d, void* buf, size_t nbytes) {
	...
	LOGF_EVERY(1024, "\tRead %u bytes from: %s\n", bytes_read,
	           Common::PathToString(file->real_name).c_str());
...
int64_t KYTY_SYSV_ABI KernelPread(int d, void* buf, size_t nbytes, int64_t offset) {
	...
	LOGF_EVERY(1024, "\tRead %u bytes (pos = %" PRId64 ") from: %s\n", bytes_read, offset,
	     Common::PathToString(file->real_name).c_str());
...
int64_t KYTY_SYSV_ABI KernelPwrite(int d, const void* buf, size_t nbytes, int64_t offset) {
	...
	LOGF_EVERY(1024, "\tWrite %u bytes (pos = %" PRId64 ") to: %s\n", bytes_written, offset,
	     Common::PathToString(file->real_name).c_str());
...
int64_t KYTY_SYSV_ABI KernelLseek(int d, int64_t offset, int whence) {
	...
	LOGF_EVERY(1024, "\tLseek (pos = %" PRId64 ") to: %s\n", offset,
	     Common::PathToString(file->real_name).c_str());
